...
## Colinearity
## Collinearity plot using Cramer's V instead of dummy variable correlation
...
## Tileplot - OTU
```{r otu_occurances_varlevels, fig.width=5.8, fig.height=3, cache=TRUE}
...
plot_logfc1 <- df_tile %>% na.omit %>% 
  filter(grepl("Education|Family income|Race/ethnicity|Age", variable)) %>%
  filter(Genus %in% levels(Genus)[1:15]) %>%
    ...
    theme(strip.text.y = element_text(angle=180, hjust=0), legend.position = c(-1,-.3), legend.direction = "horizontal",
          legend.key.size = unit(.45, "cm"))


plot_logfc2 <- df_tile %>% na.omit %>% filter(!grepl("Education|Family income|Race/ethnicity|Age", variable)) %>%
    ggplot(aes(x=Genus, y=varlevel, fill=logFC)) +
    geom_tile(color="black") + theme_minimal() +
    theme(axis.text.x = element_text(angle = 45, vjust=1, hjust = 1), strip.placement = "outside",
          panel.spacing = unit(1, "line")) +
    ...
    labs(y=NULL, x=NULL) +
    facet_grid(variable~., scale="free_y", space="free_y", switch = "y") + 
    theme(strip.text.y = element_text(angle=180, hjust=0), legend.position = c(-.6,-.6), legend.direction = "horizontal",
          legend.key.size = unit(.45, "cm"))




svg(filename = "plot_logfc1.svg", width=5.9, height=3.4)
  plot_logfc1
dev.off()

svg(filename = "plot_logfc2.svg", width=5.9, height=3)
  plot_logfc2
dev.off()

plot_logfc 

unique(df_tile$variable)
