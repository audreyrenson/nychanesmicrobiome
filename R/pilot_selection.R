...
smq <- as.matrix(jesica[, grep("SMQ_13", colnames(jesica)) ])

levi <- data.frame(key=jesica$key,
 hookah=apply(smq, 1, function(x) any(x==6)),
 ecig=apply(smq, 1, function(x) any(x==7)),
 cigar=apply(smq, 1, function(x) any(x==3)),
 never=(jesica$SMQ_1==2 & jesica$SMQ_12==2),
 cigarette=apply(smq, 1, function(x) any(x==1)) & jesica$SMQ_1==1,  #cigarettes in last 5 days & smoked 100 cigarettes in lifetime
 otherprod=apply(smq[, grep("SMQ_13_2", colnames(smq))], 1, function(x) any(x %in% 2:8)),  #other nicotine products
 stringsAsFactors = FALSE)
levi$former <- (jesica$SMQ_1==1 & jesica$SMQ_3==3 & jesica$SMQ_12==2)  #lifetime 100+ & currently not at all & no hookah/ecig/cigar
levi[is.na(levi)] <- FALSE
levi$cotinine <- jesica$Cotinine
